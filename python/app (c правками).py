"В строках 492, 493, 637 необходимо скорректировать путь для корректной работы программы."
...
def check_weekend(date): #Функция заменяет дату выходного дня на предшествующую пятницу
...
def get_next_date(date, step): #Функция проводит манипуляции с датой в зависимости от шага
    s = re.split('[- /.]+', date)
    day=int(s[0])
    mon=int(s[1])
    year=int(s[2])
            ...
            if s_mon == 2:
                if s_year % 4 == 0:
                    if  s_year % 400 == 0 or s_year % 100 != 0 and s_day < 30:
                        ...
                        for i in range(n):
                            date = get_next_date(date, step)
                            if date.weekday() == 5 and step == 'день':
                                date += timedelta(days=2)
                                date = check_weekend(date)
                            elif date.weekday() == 6 and step == 'день':
